...
import tempfile # <--- AÑADIR ESTA IMPORTACIÓN
...
# ... (El resto de las funciones: simplify_survey_column_names_llm, basic_column_simplifier, generate_variable_labels_llm, get_llm_mapping_suggestion se mantienen EXACTAMENTE IGUALES) ...
def simplify_survey_column_names_llm(column_names_list, client, model="gpt-4.1-mini"): # gpt-4.1-mini no existe aún, usando turbo
    """
...
def get_llm_mapping_suggestion(categories_list, client, model="gpt-4.1-mini"):
    if not client:
...
# ... (API Key Input, File Uploader, Options se mantienen EXACTAMENTE IGUALES) ...
# API Key Input
    ...
    try:
        ...
        if st.sidebar.button("🚀 Procesar Datos para .sav"):
            with st.spinner("Procesando datos... Por favor espera."):
                ...
                # ... (Todo el bloque de procesamiento: 1. Simplificar Nombres, 2. Generar Etiquetas, 3. Codificar Ordinales se mantiene EXACTAMENTE IGUAL)
                ...
                if do_generate_var_labels:
                    ...
                    if openai_client:
                        ...
                        original_cols_list = df_original.columns.tolist() 
                        ...
                        if not map_spss_name_to_original_question and df_processed is not None: 
                        ...
                        if generated_labels:
                            ...
                            spss_variable_labels_dict = {col: col[:256] for col in df_processed.columns} 
                    else:
                        ...
                        for i, columna_actual_spss_name in enumerate(columnas_a_evaluar):
                            ...
                            if not nombre_col_original_para_categorias:
                                if columna_actual_spss_name in df_original.columns:
                                     nombre_col_original_para_categorias = columna_actual_spss_name 
                                else: 
                            ...
                            if not (1 < num_categorias <= MAX_CATEGORIAS_PARA_LLM) :
                                if num_categorias == 0 : log_messages.append(f"  Omitiendo '{columna_actual_spss_name}': 0 categorías.")
                                elif num_categorias > MAX_CATEGORIAS_PARA_LLM: log_messages.append(f"  Omitiendo '{columna_actual_spss_name}': {num_categorias} categorías (límite {MAX_CATEGORIAS_PARA_LLM}).")
                                elif num_categorias <= 1: log_messages.append(f"  Omitiendo '{columna_actual_spss_name}': {num_categorias} categoría(s). No requiere mapeo.")
                                continue
...
if st.session_state.df_processed is not None:
    ...
    # --- MODIFICACIÓN AQUÍ para el guardado de .sav ---
    temp_file_path = None # Inicializar para el bloque finally
    try:
            ...
            if df_for_sav[col].dtype == 'object':
                try:
                    pd.to_numeric(df_for_sav[col])
                     ...
                     df_for_sav[col] = df_for_sav[col].astype(str)

        # Crear un archivo temporal para escribir el .sav
        with tempfile.NamedTemporaryFile(delete=False, suffix=".sav") as tmp_file:
            temp_file_path = tmp_file.name
        
        # Escribir el DataFrame al archivo temporal .sav
        pyreadstat.write_sav(
            df_for_sav, 
            temp_file_path, # Usar la ruta del archivo temporal
            column_labels=final_spss_variable_labels_list,
            variable_value_labels=st.session_state.spss_value_labels,
        )

        # Leer el contenido del archivo temporal en bytes para la descarga
        ...
        st.download_button(
            ...
            data=sav_bytes, # Usar los bytes leídos
        ...
        # Asegurarse de eliminar el archivo temporal si fue creado
        if temp_file_path and os.path.exists(temp_file_path):
            try:
                os.remove(temp_file_path)
            except Exception as e_del:
                st.warning(f"No se pudo eliminar el archivo temporal {temp_file_path}: {e_del}")
